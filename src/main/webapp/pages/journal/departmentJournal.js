...
//			});
	...
	var newProvince = new Ext.data.Store({
				fields : ['id', 'name']
			});
...
//			})
				...
				tbar : [{
							...
							listeners : {
								click : function() {
									...
									journalStore.on('beforeload', function(
													store, options) {
												var new_params = {
													startDate : startDate,
													endDate : endDate,
	...
	function deleteJRecords(records) {
		...
		Ext.Ajax.request({
					...
					success : function(response, options) {
						...
						if (result.success) {
							for (var i = 0; i < records.length; i++) {
								var index = journalStore.find('workId',
										records[i].get('workId'));
								if (index != -1) {
	...
	function exportJournal() {
		...
		top.Ext.Msg.confirm('提示', msg, function(btnID) {
					if (btnID == 'yes') {
						window.location.href = 'byjx/jour_exportDept.action?workIds='
								+ workIds;
					}
				});
	}
